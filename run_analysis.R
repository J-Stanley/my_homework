activities_file <- read.table("./UCI HAR Dataset/activity_labels.txt")                              ## 1
...
newx <- cbind(activitynames, x)                                                                     ## 3 
newx2 <- cbind(subjects, x)                                                                         ## 4
newx3 <- cbind(activitynames, newx2)                                                                ## 5
newx4 <- data.frame()
for(i in 1:6){
    for(j in 1:30){
